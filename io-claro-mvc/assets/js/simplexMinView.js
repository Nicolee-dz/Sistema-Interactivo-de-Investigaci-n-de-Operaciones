...
// VISTA — Genera el formulario y maneja eventos (Minimizar)
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderRestricciones(numVars, numRes) {
    ...
    for (let i = 0; i < numRes; i++) {
      ...
      // Minimizar sí tiene selector de relación
      fila += `
        <select class="form-select form-select-sm focus-min" id="rel_${i}" style="max-width:70px;">
          <option value="<=">≤</option>
          <option value=">=">≥</option>
          <option value="=">=</option>
        </select>
