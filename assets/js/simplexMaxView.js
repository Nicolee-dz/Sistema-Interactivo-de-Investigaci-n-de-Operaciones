...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function mostrarResultado(resultado) {
    ...
    box.innerHTML = `
      ...
      ${renderPasos(resultado.pasos, resultado.encabezados, 'max')}
      <hr/>
      ${renderResumenFinal(resultado, 'max')}
    `;
  }

  // ── Tablas paso a paso ──────────────────────────────────────
  function renderPasos(pasos, encabezados, tipo) {
    return pasos.map((paso, idx) => {
      ...
      return `
        <div class="mb-4">
          ...
          ${renderAnalisisPaso(paso, encabezados, tipo, esOptimo)}
          ${renderTablaPaso(paso, encabezados)}
        </div>`;
    }).join('');
  }

  function renderAnalisisPaso(paso, encabezados, tipo, esOptimo) {
    const { cj_zj, colPivote, filaPivote, base, razones, iteracion } = paso;

    if (iteracion === 0) {
      // Análisis de la tabla inicial
      const varEntra = colPivote >= 0 ? encabezados[colPivote] : '—';
      // Buscar la columna con mayor cj-zj para el siguiente paso
      let maxIdx = -1, maxVal = 0;
      cj_zj.forEach((v, i) => { if (v !== null && v > maxVal) { maxVal = v; maxIdx = i; } });
      const proxEntrada = maxIdx >= 0 ? `<strong>${encabezados[maxIdx]}</strong> (cj−Zj = ${fmt(maxVal)})` : '—';
      return `<div class="alert alert-sm mb-2 p-2" style="background:rgba(163,206,241,0.15);border:1px solid var(--color-primario);font-size:.83rem;">
    ...
    if (esOptimo) {
      const positivos = cj_zj.map((v, i) => v !== null && v <= 1e-9 ? encabezados[i] : null).filter(Boolean);
      return `<div class="alert alert-sm mb-2 p-2" style="background:rgba(201,247,245,0.4);border:1px solid var(--color-exito);font-size:.83rem;">
        <strong>✓ Solución óptima alcanzada.</strong> Todos los cj−Zj ≤ 0 (ninguna variable mejora la función objetivo).
        <br/>Base final: <strong>${base.join(', ')}</strong>
      </div>`;
    }

    const varEntra = colPivote >= 0 ? encabezados[colPivote] : '—';
    const varSale  = filaPivote >= 0 ? base[filaPivote] : '—'; // base antes del pivoteo
    const razon    = filaPivote >= 0 && razones[filaPivote] !== null ? fmt(razones[filaPivote]) : '—';
    // Buscar próxima entrante
    let maxIdx = -1, maxVal = 0;
    cj_zj.forEach((v, i) => { if (v !== null && v > maxVal) { maxVal = v; maxIdx = i; } });
    const proxInfo = maxIdx >= 0 && !esOptimo
  ...
  function renderTablaPaso(paso, encabezados) {
    ...
    const cols = encabezados.length; // sin RHS
    ...
    encabezados.forEach((enc, j) => {
      ...
      html += `<th class="${esPivotCol ? 'text-center' : 'text-center'}"
                   style="${esPivotCol ? 'background:rgba(203,180,245,0.35);' : ''}">${enc}</th>`;
    });
    ...
    tabla.forEach((fila, i) => {
      ...
      const rowStyle = esPivotFila ? 'background:rgba(203,180,245,0.18);font-weight:600;' : '';
    ...
    // Fila Cj - Zj
    html += `<tr style="background:rgba(163,206,241,0.18);">
    ...
    cj_zj.slice(0, cols).forEach((v, j) => {
      ...
      const color     = v > 0 ? 'color:#16803d;font-weight:700;' : (v < -1e-9 ? 'color:#b91c1c;' : '');
  ...
  function renderResumenFinal(resultado, tipo) {
    ...
    const filas = varsOriginales.map(([k, v]) => `
      <tr>
        ...
        <td class="text-center text-muted small">${fmt(v) === '0' ? 'No básica (no entra en la solución)' : 'Variable básica'}</td>
      </tr>`).join('');
