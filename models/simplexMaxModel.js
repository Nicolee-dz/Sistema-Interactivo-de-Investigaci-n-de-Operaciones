class simplexMaxModel extends simplexBaseModel {
  maximizar(objetivo, restricciones, rhs) {
    ...
    let cb = [], base = [];
    for (let i = 0; i < restricciones.length; i++) { cb.push(0); base.push('s' + (i + 1)); }
    ...
    return { zOptimo: zj[tabla[0].length - 1], variables: solucion, pasos, encabezados };
  }
  _capturarPaso(tabla, cb, base, funcionObj, encabezados, filaPivote, colPivote, iteracion) {
    ...
    const razones = tabla.map(fila => {
      const rhs = fila[fila.length - 1], coef = colPivote >= 0 ? fila[colPivote] : 0;
      return (coef > 0 && colPivote >= 0) ? rhs / coef : null;
    });
    return { iteracion, tabla: tabla.map(f => [...f]), cb: [...cb], base: [...base], zj: [...zj], cj_zj: [...cj_zj], razones: [...razones], filaPivote, colPivote, funcionObj: [...funcionObj], encabezados };
  }
